...
// When true, bind devices by slave index instead of on-bus alias (useful for drives that cannot set alias).
...
ECAT::ECAT(int const order){
    ...
    dc = configXML->feature("ECAT", order, "dc");
    period = configXML->attribute("ECAT", order, "period");
...
int ECAT::init(){
    ...
    if(rxPDOSwaps == nullptr || txPDOSwaps == nullptr){
        printf("allocating PDO swap lists failed\n");
        return -1;
    }
    int i = 0;
...
int ECAT::check(){
    ...
    alias2slave.clear();
    // Preserve configured aliases order for index-based matching.
    std::vector<int> configAliases;
    configAliases.reserve(alias2type.size());
...
int ECAT::config(){
    ...
    ec_slave_config_t* refClockSlaveConfig = nullptr;
        ...
        if(category == "driver"){
            // SM-PDO分配：在PreOP下通过SDO切换RxPDO 0x1600→0x1601（含PVT_KP/PVT_KD）
            unsigned char u8val = 0;
            unsigned short u16val = 0;
            unsigned int abortCode = 0;
            // RxPDO重映射: 0x1C12 从 0x1600 → 0x1601
            u8val = 0;
            if(ecrt_master_sdo_download(master, slave, 0x1C12, 0x00, &u8val, sizeof(u8val), &abortCode) < 0){
                printf("\tSDO write 0x1C12:00=0 failed, abortCode=0x%08x\n", abortCode);
            }
            u16val = 0x1601;
            if(ecrt_master_sdo_download(master, slave, 0x1C12, 0x01, (unsigned char*)&u16val, sizeof(u16val), &abortCode) < 0){
                printf("\tSDO write 0x1C12:01=0x1601 failed, abortCode=0x%08x\n", abortCode);
            }
            u8val = 1;
            if(ecrt_master_sdo_download(master, slave, 0x1C12, 0x00, &u8val, sizeof(u8val), &abortCode) < 0){
                printf("\tSDO write 0x1C12:00=1 failed, abortCode=0x%08x\n", abortCode);
            }
            printf("\tRxPDO switched to 0x1601 for slave %d\n", slave);
            // TxPDO重映射：0x1C13 从 0x1A00 → 0x1A02（含温度/电压）
            u8val = 0;
            if(ecrt_master_sdo_download(master, slave, 0x1C13, 0x00, &u8val, sizeof(u8val), &abortCode) < 0){
                printf("\tSDO write 0x1C13:00=0 failed, abortCode=0x%08x\n", abortCode);
            }
            // Step 2: 分配0x1A02到TxPDO通道1 (0x1C13:01 = 0x1A02)
            u16val = 0x1A02;
            if(ecrt_master_sdo_download(master, slave, 0x1C13, 0x01, (unsigned char*)&u16val, sizeof(u16val), &abortCode) < 0){
                printf("\tSDO write 0x1C13:01=0x1A02 failed, abortCode=0x%08x\n", abortCode);
            }
            // Step 3: 设置激活通道数为1 (0x1C13:00 = 1)
            u8val = 1;
            if(ecrt_master_sdo_download(master, slave, 0x1C13, 0x00, &u8val, sizeof(u8val), &abortCode) < 0){
                printf("\tSDO write 0x1C13:00=1 failed, abortCode=0x%08x\n", abortCode);
            }
            printf("\tTxPDO switched to 0x1A02 for slave %d\n", slave);
        }
    ...
    while(i < domainDivision.size()){
        ...
        while(j < 2){
            ...
            case 1:
                break;
            case 0:
                hands[j].rx.previous()->CurrentLimitThumb      = 1000;
                hands[j].rx.previous()->CurrentLimitThumbBend  = 1000;
                hands[j].rx.previous()->CurrentLimitForefinger = 1000;
                hands[j].rx.previous()->CurrentLimitMiddle     = 1000;
                hands[j].rx.previous()->CurrentLimitRing       = 1000;
                hands[j].rx.previous()->CurrentLimitLittle     = 1000;
                hands[j].rx.previous()->TargetSpeedThumb       = 100;
                hands[j].rx.previous()->TargetSpeedThumbBend   = 100;
                hands[j].rx.previous()->TargetSpeedForefinger  = 100;
                hands[j].rx.previous()->TargetSpeedMiddle      = 100;
                hands[j].rx.previous()->TargetSpeedRing        = 100;
                hands[j].rx.previous()->TargetSpeedLittle      = 100;
                ...
                hands[j].rx.    next()->CurrentLimitThumb      = 1000;
                hands[j].rx.    next()->CurrentLimitThumbBend  = 1000;
                hands[j].rx.    next()->CurrentLimitForefinger = 1000;
                hands[j].rx.    next()->CurrentLimitMiddle     = 1000;
                hands[j].rx.    next()->CurrentLimitRing       = 1000;
                hands[j].rx.    next()->CurrentLimitLittle     = 1000;
                hands[j].rx.    next()->TargetSpeedThumb       = 100;
                hands[j].rx.    next()->TargetSpeedThumbBend   = 100;
                hands[j].rx.    next()->TargetSpeedForefinger  = 100;
                hands[j].rx.    next()->TargetSpeedMiddle      = 100;
                hands[j].rx.    next()->TargetSpeedRing        = 100;
                hands[j].rx.    next()->TargetSpeedLittle      = 100;
                break;
            case -1:
...
void* ECAT::rxtx(void* arg){
    ...
    int i = 0, previousIncompleteness = 0;
    while(i < domainCount){
...
#if 0
                    // RS485 forwarding disabled (unused).
...
void ECAT::clean(){
    ...
    if(sdoRequestQueue.size() > 0){
        while(auto msg = sdoRequestQueue.get_nonblocking()){
            delete msg;
        }
    }
    if(sdoResponseQueue.size() > 0){
        while(auto msg = sdoResponseQueue.get_nonblocking()){
            delete msg;
        }
    }
    int i = 0;
