...
int main(int argc, char** argv) {
    ...
    // Do not start any motion (or calibration) until ALL selected motors are in CiA-402 OP (0x0037).
    sdk.getMotorActual(actuals);
    bool allOp = true;
    for (int idx : motorIdx) {
        if (!isOp(actuals[idx].statusWord)) {
    ...
    if (!allOp) {
        std::cerr << "Not all motors reached OP (0x0037); aborting.\n";
        printMotors("[NOT_OP]", waitTick, motorIdx, motorAliasForPrint, motorName, actuals, &targets);
        disableAllAndExit(sdk, motorIdx, periodUs, actuals, targets);
        return 1;
    }
