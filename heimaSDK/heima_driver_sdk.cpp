...
// Allow matching EtherCAT devices by slave index instead of alias when drives report alias=0.
bool ecatMatchByIndex = true;
...
int DriverSDK::setMotorTarget(std::vector<motorTargetStruct> const& data){
    ...
    // 第一步：CiA402 状态机处理（ControlWord / Mode）
    int i = 0;
    while(i < dofAll){
    ...
    while(i < dofAll){
        ...
        if(drivers[i].busCode == 0){
            ...
            // Position/velocity are already on output-side; do not apply gear ratio here.
            position = drivers[i].parameters.polarity * position * drivers[i].parameters.encoderResolution / 2.0 / Pi + drivers[i].parameters.countBias;
            drivers[i].rx->TargetPosition = position;
            float velocity = data[i].vel;
            velocity = drivers[i].parameters.polarity * velocity * drivers[i].parameters.encoderResolution / 2.0 / Pi;
            ...
            // CST (0x0A): 0x6071 TargetTorque is in 0.1% (per-mille) of rated current/torque.
            // In this SDK, motorTargetStruct::tor is interpreted as output-side torque (N*m).
            if(operatingMode[i] == 10){
                int torqueRaw = (int)lroundf(drivers[i].parameters.polarity * 1000.0f * torque / drivers[i].parameters.torqueConstant / drivers[i].parameters.ratedCurrent);
                int const limit = (int)maxCurrent[i];
                if(torqueRaw > limit){
                    torqueRaw = limit;
                }else if(torqueRaw < -limit){
                    torqueRaw = -limit;
                }
                drivers[i].rx->TargetTorque = (short)torqueRaw;
            }else if(operatingMode[i] == 8){
                drivers[i].rx->TargetTorque = 0;
            }else{
                // PVT (Mode 5) and other modes: torque as feedforward
                torque = drivers[i].parameters.polarity * 1000.0 * torque / drivers[i].parameters.torqueConstant / drivers[i].parameters.ratedCurrent;
                drivers[i].rx->TargetTorque = torque;
            }
            // PVT/MIT模式 (Mode 5): 写入 kp/kd (× 1000)
            if(operatingMode[i] == 5){
                drivers[i].rx->PvtKp = (int)lroundf(data[i].kp * 1000.0f);
                drivers[i].rx->PvtKd = (int)lroundf(data[i].kd * 1000.0f);
            }else{
                drivers[i].rx->PvtKp = 0;
                drivers[i].rx->PvtKd = 0;
            }
            ...
            *reinterpret_cast<float*>(&drivers[i].rx->TargetVelocity) = velocity;
            ...
            drivers[i].rx->TargetTorque = single2half(torque);
            drivers[i].rx->ControlWord = single2half(data[i].kp);
            drivers[i].rx->PvtKp = single2half(data[i].kd);
...
int DriverSDK::getMotorActual(std::vector<motorActualStruct>& data){
    ...
    while(i < dofAll){
        ...
        if(drivers[i].busCode == 0){
            // TxPDO uses output-side counts:
            //  - position: counts, where encoderResolution counts = 1 revolution
            //  - velocity: counts/s, RPM = X * 60 / encoderResolution
            // 0x6077 ActualTorque is in 0.1% (per-mille) of rated current/torque.
            data[i].pos = 2.0 * Pi * drivers[i].parameters.polarity * (drivers[i].tx->ActualPosition - drivers[i].parameters.countBias) / drivers[i].parameters.encoderResolution;
            data[i].vel = 2.0 * Pi * drivers[i].parameters.polarity * drivers[i].tx->ActualVelocity / drivers[i].parameters.encoderResolution;
            data[i].tor = drivers[i].parameters.polarity * drivers[i].tx->ActualTorque / 1000.0f * drivers[i].parameters.ratedCurrent * drivers[i].parameters.torqueConstant;
            // Temperature & voltage via TxPDO 0x1A02 (no SDO polling needed)
            data[i].temp      = drivers[i].tx->MotorTemperature;
            data[i].driveTemp = drivers[i].tx->DriveTemperature;
            data[i].voltage   = drivers[i].tx->Voltage;
