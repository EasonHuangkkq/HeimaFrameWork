...
- 每周期写 `MaxTorque= maxCurrent`。CST：`TargetTorque`=0.01A 原始值，限幅 ±`maxCurrent`。  
...
- 10: 0x6071 TargetTorque (S16) ← 指令电流位
...
3) 读寄存器：  
   ...
   - `sudo /usr/local/bin/ethercat upload -p0 0x6071 0` 应等于命令（如 300=3A）  
   - `sudo /usr/local/bin/ethercat upload -p0 0x6072 0` 应等于限流（默认 1000=10A）  
