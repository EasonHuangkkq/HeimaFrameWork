...
ConfigXML::ConfigXML(char const* file){
    ...
    auto rc = xmlDoc.LoadFile(file);
    if(rc != tinyxml2::XML_SUCCESS){
        printf("loading config xml %s failed: %s\n", file, xmlDoc.ErrorStr());
        exit(-1);
    }
    if(xmlDoc.FirstChildElement("Config") == nullptr){
        printf("invalid config xml %s: missing <Config> root\n", file);
        exit(-1);
    }
}
