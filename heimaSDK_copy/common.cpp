...
MotorParameters::MotorParameters(){
    ...
    minimumPosition = -1.0;
    sdoTemplate = SDOMsg{nullptr, 0, 0, 0, 0x0000, 0x00, 0, 0, 0, 0};
    temperatureSDO = SDOMsg{nullptr, 0, 0, 0, 0x0000, 0x00, 0, 0, 0, 0};
    clearErrorSDO = SDOMsg{nullptr, 0, 0, 0, 0x0000, 0x00, 0, 0, 0, 0};
}
...
int MotorParameters::load(std::string const& bus, int const alias, std::string const& type, ec_sdo_request_t* const sdoHandler){
    ...
    if(bus == "ECAT"){
        ...
        // ===============================================
        // [FIX] Commented out to prevent crash with MT_Device
        // config.xml does not have "Temperature" or "ClearError" for MT_Device
        // ===============================================
        /*
