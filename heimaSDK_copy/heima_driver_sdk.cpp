     ...
     while(i < dofAll){
         ...
         if(drivers[i].busCode == 0){
             ...
             if(operatingMode[i] == 10){
                 ...
                 // TorqueOffset shares the same field (macro alias); avoid zeroing TargetTorque.
                 // printf("[DBG] tx-node %p 6071(raw)=%d\n", imp.ecats[drivers[i].order].rxPDOSwaps[drivers[i].domain]->nodePtr.load(), torqueRaw);
             }else{
                 if(torque > drivers[i].parameters.maximumTorque){
                     torque = drivers[i].parameters.maximumTorque;
                 }else if(torque < -drivers[i].parameters.maximumTorque){
                 ...
                 // 0x6072 MaxTorque (U16) is never set elsewhere; keep it non-zero so CST torque commands take effect.
                 drivers[i].rx->MaxTorque = maxCurrent[i];
 ...
 int DriverSDK::getMotorActual(std::vector<motorActualStruct>& data){
     ...
     while(i < dofAll){
         ...
         if(drivers[i].busCode == 0){
             ...
             // Temperature SDO optional; skip if not configured
             if(drivers[i].sdoHandler != nullptr && drivers[i].parameters.temperatureSDO.index != 0x0000){
