...
int DriverSDK::setMotorTarget(std::vector<motorTargetStruct> const& data){
    ...
    while(i < dofAll){
        ...
        if(drivers[i].busCode == 0){
            ...
            // MT_Device reports/accepts output-side 17-bit counts (131072 counts/rev), so do not apply gear ratio here.
            if(drivers[i].type == "MT_Device"){
            ...
            // For CST (0x0A) MT_Device expects current in 0.01A units (e.g. 100 = 1A).
            // In this SDK, for MT_Device+CST we interpret motorTargetStruct::tor as current in A.
            // For other devices/modes keep the legacy scaling.
            if(operatingMode[i] == 10){
                int torqueRaw = 0;
                if(drivers[i].type == "MT_Device"){
                    float const currentA = torque;
                    torqueRaw = (int)lroundf(drivers[i].parameters.polarity * currentA * 100.0f);
                }else{
                    torqueRaw = (int)lroundf(drivers[i].parameters.polarity * torque);
                }
...
int DriverSDK::getMotorActual(std::vector<motorActualStruct>& data){
    ...
    while(i < dofAll){
        ...
        if(drivers[i].busCode == 0){
            if(drivers[i].type == "MT_Device"){
                // MT_Device TxPDO uses output-side 17-bit counts:
                //  - position: counts, where 131072 counts = 1 revolution
                //  - velocity: counts/s, RPM = X * 60 / 131072
                // MT_Device torque feedback is current in 0.01A units; output torque = I(A) * Kt_out.
                ...
                float currentA = drivers[i].parameters.polarity * drivers[i].tx->ActualTorque / 100.0f;
                data[i].tor = currentA * drivers[i].parameters.torqueConstant;
            }else{
