...
//   leg_smoke_test.cpp heima_driver_sdk.cpp ecat.cpp config_xml.cpp common.cpp tinyxml2.cpp rs232.cpp rs485.cpp can.cpp \
//   -I. -I/usr/local/include -L/usr/local/lib -lethercat -lm \
//   -o build/leg_smoke_test
//
// Run examples:
//   # 1) Just monitor (no commands sent):
//   sudo ./build/leg_smoke_test --config config_leg.xml --limb 4
//
//   # 2) Enable and hold current position (safe default):
//   sudo ./build/leg_smoke_test --config config_leg.xml --limb 4 --enable
//
//   # 3) Enable and do a small sine test on joint 0:
//   sudo ./build/leg_smoke_test --config config_leg.xml --limb 4 --enable --joint 0 --amp 0.05 --freq 0.2
//
// Notes:
// - This program targets leg limbs (limb 4 = current left leg, limb 0 = future right leg).
...
void printUsage(const char* argv0) {
    std::cout
        << "Usage: " << argv0 << " [--config config_leg.xml] [--limb 4] [--aliases a1,a2,...]\n"
        << "              [--mode 8] [--period-us 1000] [--seconds 10]\n"
        << "              [--enable] [--follow-actual] [--settle-ms 0]\n"
        << "              [--joint j] [--amp rad] [--freq hz]\n"
        << "              [--print-ms 200] [--max-current u16]\n"
        << "\n"
        << "  --config       Path to config_leg.xml (default: config_leg.xml)\n"
        << "  --limb         Limb index in config_leg.xml <Motors> (default: auto)\n"
        << "                (Leg layout: 0=right leg, 4=left leg)\n"
        ...
        << "  --joint        Leg joint index within selected limb (0-based); enables sine test\n"
...
int main(int argc, char** argv) {
    std::string configPath = "config_leg.xml";
    int limb = -1;
    ...
    if (!aliasesCsv.empty()) {
        ...
        if (limb < 0) {
            const int preferOrder[] = {4, 0, 1, 2, 3, 5};
        ...
        if (limb < 0 || limb >= static_cast<int>(motorAlias.size())) {
            std::cerr << "No limb motors found in config_leg.xml. Use --aliases to specify motors.\n";
            return 1;
        }
    ...
    if (motorIdx.empty()) {
        std::cerr << "No motors selected. Check config_leg.xml limb mapping or use --aliases.\n";
        return 1;
    }
