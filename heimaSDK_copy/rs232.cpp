...
RS232::RS232(char const* device, int const baudrate, char const* type){
    ...
    if(strcmp(type, "Xsens") == 0){
        ...
        frameLength = sizeof(YesenseImuFrame);
        header0 = 0x59;
        header1 = 0x53;
        valid = nullptr;
        rpy0 = rpy0yesense;
        ...
        acc2 = acc2yesense;
        useYesense = true;
        yesenseDecoder = new YesenseImuDecoder();
    }else{
...
void* RS232::recv(void* arg){
    ...
    if(imu->useYesense){
        ...
        opt.c_cc[VTIME] = 1; // 100ms timeout
        opt.c_cc[VMIN] = 0;
        ...
        static struct timespec lastReport = {0, 0};
        while(true){
            int n = read(fd, buff, sizeof(buff));
            if(n <= 0){
                continue;
            }
            YesenseImuFrame frame{};
            if(imu->yesenseDecoder->feed(buff, (size_t)n, frame)){
                memcpy(imu->txSwap->nodePtr.load()->next->memPtr, &frame, sizeof(frame));
                imu->txSwap->advanceNodePtr();
                if(lastReport.tv_sec == 0 && lastReport.tv_nsec == 0){
                    clock_gettime(CLOCK_MONOTONIC, &lastReport);
                }
                frameCount++;
                struct timespec now;
                clock_gettime(CLOCK_MONOTONIC, &now);
                long long elapsedNs = (now.tv_sec - lastReport.tv_sec) * 1000000000LL +
                    (now.tv_nsec - lastReport.tv_nsec);
                if(elapsedNs >= 1000000000LL){
                    double seconds = (double)elapsedNs / 1000000000.0;
...
RS232::~RS232(){
    ...
    if(yesenseDecoder != nullptr){
        delete yesenseDecoder;
        yesenseDecoder = nullptr;
    }
    if(device != nullptr){
        free(device);
